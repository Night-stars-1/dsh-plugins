...
**Remote admin (`allowRemoteAdmin`).** dsh pins only its genuinely privileged `/api` methods — `settings.*`, `credentials.*`, `host.pickDirectory`/`host.openPath`, `llm.discoverModels`, and preset authoring (`agentPreset.read`/`copy`/`openDocument`/`remove`) — to a loopback Host; behind a reverse proxy those answer `HTTP 403` for remote users. Ordinary methods (`agentPreset.list`/`select`, `llm.providers`/`llm.models`, chat) are **not** pinned — they only need the serving authority declared in `trustedHosts` (`DSH_TRUSTED_HOSTS` in the Docker image), so a remote `agentPreset.list` 403 is fixed by `trustedHosts`, not by this flag. Set `allowRemoteAdmin: true` to open the privileged plane to **authenticated** sessions: after the guard confirms the session cookie, it presents a loopback authority (Host and Origin) to the downstream handler for `/api` requests. This deliberately trades away dsh's loopback safety default, so weigh it — every key holder can then read credentials (model API keys) and change configuration. Key management (`/api/auth`) is exempt from the rewrite and always judges the real Host, so adding or removing access keys stays host-machine-only regardless. Alternatives that need no rewrite: SSH port-forward (`ssh -L 8080:127.0.0.1:8080 server`) or Tailscale, which present a genuine loopback/private Host and unlock the privileged plane with the flag off.
